...
//            ->get();
    ...
    public function CasualAcademicStore(Request $request){
        ...
        if(DB::table('user')->where('user_id', $user_id)->where('user_is_available', 1)->where('user_is_casual_academic', 1)->exists()){
            return redirect("/casualAcademic")
                ->with(['notify' => true,
                    'type' => 'success',
                    'title' => 'Success!',
                    'message' => 'The User already is a Casual Academic!']);
        }
...
//            DB::commit();
    ...
    public function CasualAcademicDelete(Request $request){
        // Check login status
        if(!Session::has('login')){
            return loginExpired();  // Have not logged in, redirect to the login page.
        }
        ...
        try{
            ...
            DB::table('user')
                ->where('user_id', $user_id)
                ->update(['user_is_casual_academic' => 0,
                    'user_last_edit_user' => Session::get('user_id'),
                    'user_last_edit_time' => date('Y-m-d H:i:s'),
                ]);
        }
